...
class XMLGenerator:
    ...
    def xml_generator(self):
        ...
        filtered_sheet_names = [sheet for sheet in sheet_names if sheet.lower() != 'config']
        xml_text = ''
        joint_index = 0
        space_dict = {}
        # 遍历所有工作表
        for index, sheet_name in enumerate(filtered_sheet_names):
...
class DataFileHandler(FileSystemEventHandler):
    def __init__(self, node, data_path, debounce_interval=1.0):
        super().__init__()
        self.node = node
    ...
    def on_modified(self, event):
        ...
        with self.lock:
            if current_time - self.last_modified < self.debounce_interval:
                self.node.get_logger().info("忽略短时间内的重复修改事件")
                return
            self.last_modified = current_time

        self.node.get_logger().info(f"检测到文件修改: {event.src_path}")
        try:
            self.node.urdf_generator.urdf_generator()
            self.node.update_robot_description()
            self.node.xml_generator.xml_generator()
        except Exception as e:
            self.node.get_logger().error(f"处理文件变化时出错: {e}")

class WatchdogObserver:
    def __init__(self, node, data_file_path):
        self.node = node
        ...
        self.event_handler = DataFileHandler(node, self.data_file_path)

    def start(self):
        directory = os.path.dirname(self.data_file_path)
        self.observer.schedule(self.event_handler, path=directory, recursive=False)
        self.observer.start()
        self.node.get_logger().info(f"已启动 watchdog 观察者，监控路径: {self.data_file_path}")

    def stop(self):
        if self.observer.is_alive():
            self.observer.stop()
            self.observer.join()
            self.node.get_logger().info("已停止 watchdog 观察者。")
...
class URDFGeneratorNode(Node):
    def __init__(self):
        ...
        try:
            self.watchdog = WatchdogObserver(self, data_file_path)
        except FileNotFoundError as e:
            self.get_logger().error(str(e))
            return

        # 启动 watchdog 观察者在单独的线程
        self.watchdog_thread = threading.Thread(target=self.watchdog.start, daemon=True)
